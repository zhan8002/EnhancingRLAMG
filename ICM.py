...
class ICM(nn.Module):
    ...
    def __init__(self, state_dim=2381, encoding_size=256, num_layers=2, action_dim=6, activation=nn.Tanh()): # 1048576
        super().__init__()
        self.act_dim = action_dim
        # Encoder
        layers = list()
        layers.append(nn.Linear(2381, encoding_size))
    ...
    def forward(self, act, curr_obs, next_obs, mask):
        ...
        out = self.act_i1(out)
        #pred_act = torch.transpose(self.fc_i2(out), 0, 1)
