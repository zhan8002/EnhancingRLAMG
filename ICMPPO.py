...
class ICMPPO:
    def __init__(self, writer, state_dim=2381, action_dim=6, n_latent_var=64, lr=1e-3, betas=(0.9, 0.999),
                 gamma=0.99, ppo_epochs=4, icm_epochs=1, eps_clip=0.2, ppo_batch_size=32,
                 icm_batch_size=16, intr_reward_strength=0.01, lamb=0.95, device='cpu'): #0.02
        self.lr = lr
        self.betas = betas
        self.gamma = gamma
        self.lambd = lamb
    ...
    def update(self, memory, timestep):
        ...
        for _ in range(self.ppo_epochs):
            ...
            for i in range(0, len(indexes), self.ppo_batch_size):
                ...
                decay_epsilon = linear_decay_eps(self.timestep)
                decay_beta = linear_decay_beta(self.timestep)
                ...
                loss.backward()
                self.optimizer.step()
                linear_decay_lr(self.optimizer, self.timestep)
    ...
    def _icm_update(self, epochs, batch_size, curr_states, next_states, actions, mask):
        ...
        for _ in range(epochs):
            ...
            for i in range(0, len(indexes), batch_size):
                ...
                linear_decay_lr(self.optimizer_icm, self.timestep)
